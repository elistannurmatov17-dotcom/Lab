...
class Application(Base):
    ...
    lk_password_encrypted: Mapped[str] = mapped_column(Text)
    inn: Mapped[str] = mapped_column(String(14), index=True)
    company: Mapped[str] = mapped_column(String(255))
    legal_address: Mapped[str] = mapped_column(Text)
    director: Mapped[str] = mapped_column(String(255))
    director_inn: Mapped[str] = mapped_column(String(14))
    director_phone: Mapped[str] = mapped_column(String(32))
    director_email: Mapped[str] = mapped_column(String(255))
    extra_phone: Mapped[str] = mapped_column(String(32))
    extra_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ...
    tax_regime: Mapped[str] = mapped_column(Text)
    vat: Mapped[bool] = mapped_column(Boolean, default=False)
    calc_types: Mapped[list] = mapped_column(JSONB)
    comment: Mapped[str | None] = mapped_column(Text, nullable=True)
    manager_id: Mapped[str | None] = mapped_column(UUID(as_uuid=False), ForeignKey("managers.id"), nullable=True, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, index=True)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, onupdate=utcnow)
    manager: Mapped[Manager | None] = relationship(back_populates="applications")
...
class Document(Base):
    ...
    __table_args__ = (UniqueConstraint("application_id", "document_type", name="uq_application_document_type"),)
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, default=lambda: str(uuid4()))
    application_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey("applications.id", ondelete="CASCADE"), index=True)
...
class AuditLog(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, default=lambda: str(uuid4()))
    application_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey("applications.id", ondelete="CASCADE"), index=True)
    actor_manager_id: Mapped[str | None] = mapped_column(UUID(as_uuid=False), ForeignKey("managers.id"), nullable=True)
