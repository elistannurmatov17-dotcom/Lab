...
from .schemas import ApplicationDetails, ApplicationListItem, AssignManager, ManagerOut, StatusUpdate, TokenResponse
...
def login(username: str = Form(...), password: str = Form(...), db: Session = Depends(get_db)):
    m = db.scalar(select(Manager).where(Manager.username == username))
    if not m or not verify_password(password, m.password_hash):
        raise HTTPException(401, "Неверный логин или пароль")
    return TokenResponse(access_token=create_access_token(m.id))
...
def applications(
    ...
    if manager_id:
        q = q.where(Application.manager_id == manager_id)
...
def update_status(
    ...
    db.add(
        AuditLog(
            ...
            details={"from": old, "to": payload.status, "comment": payload.comment},
        )
    )
    db.commit()
    db.refresh(item)
...
def assign(
    ...
    db.add(
        AuditLog(
            ...
            actor_manager_id=manager.id,
            action="ASSIGNED",
            details={"manager_id": assignee.id},
        )
    )
    db.commit()
    db.refresh(item)
...
def managers(manager=Depends(get_current_manager), db: Session = Depends(get_db)):
    return list(
        db.scalars(select(Manager).where(Manager.is_active == True).order_by(Manager.username)).all()
    )
...
def document(
    ...
    path = Path(s.upload_dir) / doc.storage_name
    if not path.is_file():
